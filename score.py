...
from kernel import gauss_kernel_gram, median_heuristic, exp_kernel_gram, silverman
...
def stein_score(x, reg, score_kernel = "gauss"):
    """
...
def kde(x, reg = 0.01, score_kernel = "gauss", x_eval = None):
    """
...
def stein_score_all(x, y, reg, score_kernel = "gauss"):
    """
    ...
    sx = stein_score(x, reg, score_kernel[0])
    sy = stein_score(y, reg, score_kernel[1])
    sxy = stein_score(xy, reg, score_kernel[2])

    return sx, sy, sxy

def kde_score_all(x, y, reg, score_kernel = "gauss"):
    """
    ...
    sx, kde_x = kde(x, reg, score_kernel[0])
    sy, kde_y = kde(y, reg, score_kernel[1])
    sxy, kde_xy = kde(xy, reg, score_kernel[2])

    return sx, sy, sxy

def hybrid_score_all(x, y, reg, score_kernel = "gauss"):
    """
    ...
    sx, kde_x = kde(x, reg, score_kernel[0])
    sy, kde_y = kde(y, reg, score_kernel[1])
    sxy = stein_score(xy, reg*20, score_kernel[2])

    return sx, sy, sxy

def score_matching(x, hidden_size = 16, lr = 0.1, lam = 1, epochs = 50, seed = 0, denoising = False, sigma = 0.1, init_weight = 0.2, n_reinit=1):
    """
    (not currently used)
    ...
    for i in range(n_reinit):
        ...
        params = {'w1': jax.random.normal(shape=(d, hidden_size), key=jax.random.PRNGKey(seed + 0)) * init_weight,
            ...
            'w2': jax.random.normal(shape=(hidden_size, 1), key=jax.random.PRNGKey(seed + 2)) * init_weight,
            'b2': jax.random.normal(shape=(), key=jax.random.PRNGKey(seed + 3)) * init_weight}
